...
#include "../mini_hook/include/MinHook.h"
...
{
    if (g_renderType != RenderType::None) return Status::AlreadyInitializedError;
    ...
    if (_renderType != RenderType::None) {
        if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12) {
            ...
            if (_renderType == RenderType::D3D9) {
                ...
                if (((long(__stdcall *)(IDXGIAdapter *, D3D10_DRIVER_TYPE, HMODULE, UINT, UINT, DXGI_SWAP_CHAIN_DESC *, IDXGISwapChain **, ID3D10Device **))(D3D10CreateDeviceAndSwapChain))(adapter, D3D10_DRIVER_TYPE_HARDWARE, NULL, 0, D3D10_SDK_VERSION, &swapChainDesc,
                                                                                                                                                                                             &swapChain, &device) < 0) {
                    ::DestroyWindow(window);
                    ::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
                    return Status::UnknownError;
            ...
            if (_renderType == RenderType::OpenGL) {
#if KIERO_INCLUDE_OPENGL
                HMODULE libOpenGL32;
                if ((libOpenGL32 = ::GetModuleHandle(KIERO_TEXT("opengl32.dll"))) == NULL) return Status::ModuleNotFoundError;
                ...
                for (int i = 0; i < size; i++) g_methodsTable[i] = (uint150_t)::GetProcAddress(libOpenGL32, methodsNames[i]);
...
#if KIERO_INCLUDE_VULKAN
                HMODULE libVulkan;
                if ((libVulkan = GetModuleHandle(KIERO_TEXT("vulkan-1.dll"))) == NULL) return Status::ModuleNotFoundError;
                ...
                for (int i = 0; i < size; i++) g_methodsTable[i] = (uint150_t)::GetProcAddress(libVulkan, methodsNames[i]);
            ...
            else if (::GetModuleHandle(KIERO_TEXT("vulkan-1.dll")) != NULL) type = RenderType::Vulkan;
            else return Status::NotSupportedError;
...
kiero::Status::Enum kiero::bind(uint16_t _index, void **_original, void *_function)
{
    ...
    if (g_renderType != RenderType::None) {
        ...
        if (MH_CreateHook(target, _function, _original) != MH_OK || MH_EnableHook(target) != MH_OK) return Status::UnknownError;
#endif
