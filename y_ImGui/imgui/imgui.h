...
{
    ImGuiWindowFlags_None                   = 0,        // 去掉 标题文字
    ImGuiWindowFlags_NoTitleBar             = 1 << 0,   // 去掉 标题栏
    ImGuiWindowFlags_NoResize               = 1 << 1,   // 去掉 右下角小三角，禁止调整窗口大小
    ImGuiWindowFlags_NoMove                 = 1 << 2,   // 禁止移动窗口位置
    ImGuiWindowFlags_NoScrollbar            = 1 << 3,   // 去掉 滚动条（依然可以用滚轮 操作上下滚动）
    ImGuiWindowFlags_NoScrollWithMouse      = 1 << 4,   // 禁用 鼠标滚轮对窗口上下的操作
    ImGuiWindowFlags_NoCollapse             = 1 << 5,   // 去掉窗口左上角 的小三角，禁用菜单栏收起功能
    ImGuiWindowFlags_AlwaysAutoResize       = 1 << 6,   // 根据 窗口 内容自动调整窗口大小
    ImGuiWindowFlags_NoBackground           = 1 << 7,   // 不显示窗口背景
    ImGuiWindowFlags_NoSavedSettings        = 1 << 8,   // 永远不要在. ini 文件中加载/保存设置
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // 禁止鼠标对窗口操作
    ImGuiWindowFlags_MenuBar                = 1 << 10,  // 图形界面标志菜单栏
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // 横向滚动条
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // 当从隐藏状态转换到可见状态时，禁用获取焦点
    ImGuiWindowFlags_NoBringToFrontOnFocus  = 1 << 13,  // 关闭在获取焦点时将窗口向前移动的功能(例如，单击它或通过编程使它获得焦点)
    ...
    ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 16,  // 确保没有边框的子窗口使用样式
    ImGuiWindowFlags_NoNavInputs            = 1 << 18,  // 窗口中没有游戏板/键盘导航
    ImGuiWindowFlags_NoNavFocus             = 1 << 19,  // 不能通过游戏板/键盘导航对这个窗口进行聚焦(例如，通过 CTRL + TAB 跳过)
    ...
    // [内部]
    ImGuiWindowFlags_NavFlattened           = 1 << 23,  // [ BETA ]在子窗口上: 允许游戏板/键盘导航跨越父窗口边界到这个子窗口或兄弟子窗口之间。
    ImGuiWindowFlags_ChildWindow            = 1 << 24,  // 不要使用! 仅供 BeginChild ()内部使用
    ImGuiWindowFlags_Tooltip                = 1 << 25,  // 不要使用! 仅供 BeginTooltip ()内部使用
    ImGuiWindowFlags_Popup                  = 1 << 26,  // 不要使用! 仅供 BeginPopup ()内部使用
    ImGuiWindowFlags_Modal                  = 1 << 27,  // 不要使用! 仅供 BeginPopupModal ()内部使用
    ImGuiWindowFlags_ChildMenu              = 1 << 28,  // 不要使用! 仅供 BeginMenu ()内部使用
};
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CharsHexadecimal    = 1 << 1,   // 允许0123456789ABCDEFabc 定义
    ImGuiInputTextFlags_CharsUppercase      = 1 << 2,   // Turn a..z into A..Z
    ImGuiInputTextFlags_CharsNoBlank        = 1 << 3,   // 过滤掉空格、制表符
    ImGuiInputTextFlags_AutoSelectAll       = 1 << 4,   // 选择整个文本时，第一次采取鼠标焦点
